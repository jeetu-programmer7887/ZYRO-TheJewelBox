...
const Rings = () => {
  ...
  const allRings = useMemo(() => {
    return allProducts?.filter(item => item.category === 'Ring') || [];
  }, [allProducts]);

  const visibleRings = useMemo(() => {
    return allRings.slice(0, visibleCount);
  }, [allRings, visibleCount]);

  const loadMore = () => {
    if (visibleCount < allRings.length && !isIncrementing) {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && visibleCount < allRings.length) {
          loadMore();
        }
      },
      { threshold: 0.1 }
  ...
  }, [visibleCount, allRings.length]);

  return (
    <>
      <HeroSection
        bgImage="/images/RingHero.jpg"
        title={"RINGS"}
      ...
      <div className="content relative">
        ...
        <div ref={loaderRef} className="flex flex-col items-center justify-center py-16">
          ...
          {!isIncrementing && visibleCount >= allRings.length && allRings.length > 0 && (
