...
const Earrings = () => {
  ...
  // 1. State for pagination and loading
  const [visibleCount, setVisibleCount] = useState(12);
  const [isIncrementing, setIsIncrementing] = useState(false);
  const loaderRef = useRef(null);

  // Filter all earring products first
  const allEarrings = useMemo(() => {
    return allProducts?.filter(item => item.category === 'Earring') || [];
  }, [allProducts]);

  // 2. Slicing the array to show only the visible portion
  const visibleEarrings = useMemo(() => {
    return allEarrings.slice(0, visibleCount);
  }, [allEarrings, visibleCount]);

  // 3. Logic to "load" more data
  const loadMore = () => {
    if (visibleCount < allEarrings.length && !isIncrementing) {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && visibleCount < allEarrings.length) {
          loadMore();
        }
      },
      { threshold: 0.1 }
  ...
  }, [visibleCount, allEarrings.length]);

  return (
    <>
      <HeroSection
        bgImage="/images/earring.jpg"
        title={"EARRINGS"}
        subtitles={["Elevate Your Style with Every Dangle"]}
        topTitle={"60"}
        showTyping={true}
        showCursor={true}
        typeSpeed={40}
      ...
      <div className="content relative">
        ...
        <div className="cardsContainer mx-auto w-full max-w-360 px-2 md:px-8 lg:px-6 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {allEarrings.length > 0 ? (
            visibleEarrings.map((item, index) => (
              <Cards
                key={item._id || index}
                product={{
                  _id: item._id,
        ...
        {/* 5. The Infinite Loader Sentinel */}
        <div ref={loaderRef} className="flex flex-col items-center justify-center py-16">
          ...
          {!isIncrementing && visibleCount >= allEarrings.length && allEarrings.length > 0 && (
