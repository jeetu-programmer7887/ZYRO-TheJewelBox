...
const UserAccount = () => {
  ...
  const navOptions = [
    { name: "MY ACCOUNT", path: "/userAccount" },
    { name: "ORDERS", path: "/userAccount/order" },
    { name: "CANCELLED", path: "/userAccount/cancel" },
    { name: "RETURNS", path: "/userAccount/returns" },
    { name: "COINS", path: "/userAccount/coins" },
  ];
  ...
  const sidebarLinkCSS = ({ isActive }) =>
    `py-6 px-10 xl:px-20 transition-all duration-200 border-b border-gray-400 hover:bg-(--color-green) hover:text-(--color-gold) ${
      isActive ? "text-(--color-gold) bg-(--color-green)" : ""
    }`;

  const currentOptionName = navOptions.find(opt => opt.path === location.pathname)?.name || "MY ACCOUNT";

  return (
    <div className="flex flex-col lg:flex-row pt-16 lg:pt-20 min-h-screen">
      
      {/* Sidebar - Desktop */}
      <div className="hidden lg:flex flex-col lg:w-1/4 border-r bg-(--color-lightgreen) border-gray-400">
        <div className="name py-9 px-10 xl:px-20 border-b border-gray-400">
          <h1 className="text-2xl font-bold text-(--color-gold) truncate tracking-tight">
            {user?.firstName + " " + user?.lastName}
          </h1>
          <p className="text-gray-500 text-sm truncate mt-1 font-normal">{user?.email}</p>
        </div>
        <nav className="flex flex-col">
          {navOptions.map((opt) => (
            <NavLink key={opt.path} to={opt.path} end={opt.path === "/userAccount"} className={sidebarLinkCSS}>
              {opt.name}
            </NavLink>
          ))}
        </nav>
            ...
            >
              {navOptions.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-6 py-4 text-sm transition-colors border-b last:border-none border-gray-50 ${
      ...
      <main className="content w-full lg:w-3/4">
